...
function plotData(gymName::String, firstDay::Date, lastDay::Date, dayOpts::Array{Int,1})
...
function plotBWData(dataEval::DataEvalType)
    ...
    p = plot(dataEval.dataArray[1].time, dataEval.dataArray[1].percent, linecolor = cols[Dates.dayofweek(dataEval.TimeEval.evalDayArray[1])], label = Dates.dayname(dataEval.TimeEval.evalDayArray[1]))
    plot!(p, dataEval.dataArray[1].time, replaceMissingByZero(dataEval.dataArray[1].waiting), linecolor = cols[Dates.dayofweek(dataEval.TimeEval.evalDayArray[1])], linestyle=:dash, label="waiting")
...
function plotKBData(dataEval::DataEvalType)
    ...
    # Plot frei_klettern
    p = plot(dataEval.dataArray[1].time, dataEval.dataArray[1].frei_klettern, linecolor = cols[Dates.dayofweek(dataEval.TimeEval.evalDayArray[1])], label = Dates.dayname(dataEval.TimeEval.evalDayArray[1]))
    ...
    n = readline()
    # Plot frei_bouldern
    q = plot(dataEval.dataArray[1].time, dataEval.dataArray[1].frei_bouldern, linecolor = cols[Dates.dayofweek(dataEval.TimeEval.evalDayArray[1])], label = Dates.dayname(dataEval.TimeEval.evalDayArray[1]), reuse=false)
